...
//        }
        ...
        if (remoteMessage.getData() != null) {
            ...
            String sender_id = remoteMessage.getData().get("user_id");
            String receiver_id = remoteMessage.getData().get("receiver_id");
            String sender_name = remoteMessage.getData().get("user_name");
            String msg_date = remoteMessage.getData().get("msg_date");
            ...
            if (!(Integer.valueOf(sender_id) == HomeActivity.userModel.getId())) {

                // check if app in background or not
                if (isAppIsInBackground(this)) {
                    // app is in background show notification to user
//                        sendNotification(message);
                    NotificationHelper.getInstance(this).createNotification(message);
                } else {
